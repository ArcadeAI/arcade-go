...
func TestAdminUserConnectionListWithOptionalParams(t *testing.T) {
...
func TestAdminUserConnectionDelete(t *testing.T) {
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	...
	err := client.Admin.UserConnections.Delete(context.TODO(), "id")
	if err != nil {
		var apierr *arcadego.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
